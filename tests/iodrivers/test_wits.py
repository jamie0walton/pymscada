...
async def test_wits_api():
    ...
    if not url:
        pytest.skip("MSCADA_WITS_URL environment variable not set")
    api_config = {
        'url': url,
        'client_id': client_id,
    ...
    tag = Tag('MAT1101', float)
    tag.add_callback(callback, BUS_ID)
    client = WitsAPIClient(bus_ip=None, api=api_config)
    try:
